<?php if (is_single() || is_page()) : ?>
	<article id="article-<?php the_ID(); ?>" <?php post_class(); ?>>
		<?php $url_miniature = wp_get_attachment_image_src(get_post_thumbnail_id(), 'full'); ?>
		<div class="couverture" style="background-image: url('<?php echo $url_miniature[0]; ?>');">
			<header>
				<h1><?php the_title(); ?></h1>
				<aside>
...
<?php else : ?>
	<article id="article-<?php the_ID(); ?>" <?php post_class(); ?>>
		<?php $url_miniature = wp_get_attachment_image_src(get_post_thumbnail_id(), 'full'); ?>
		<a href="<?php echo the_permalink(); ?>"><div class="miniature-article" style="background-image: url('<?php echo $url_miniature[0]; ?>');"></div></a>
		<header>
